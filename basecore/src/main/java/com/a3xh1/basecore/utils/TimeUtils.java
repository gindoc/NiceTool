...
import static com.a3xh1.basecore.utils.ConstUtils.*;
...
public class TimeUtils {
     ...
     * @return
     */
    public static String getTimeFormatText(long timeMillure) {
        Date date = new Date(timeMillure);
        if (date == null) {
            return null;
        }
    ...
    public static String getRestTime(Context context, long milliseconds) {
        long interval = new Date(milliseconds).getTime() - new Date().getTime();
        String format = context.getString(R.string.rest_time);
        if (interval <= 0) return String.format(format, 0, 0, 0, 0);
        int days = 0, hours = 0, minutes = 0, seconds = 0;
        if (interval / (24 * 3600000) > 0) {
            days = (int) (interval / (24 * 3600000));
            interval %= 24 * 3600000;
        }
        if (interval / 3600000 > 0) {
            hours = (int) (interval / 3600000);
            interval %= 3600000;
        }
        if (interval / 60000 > 0) {
